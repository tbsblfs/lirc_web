...
describe('lirc_web', function () {
    ...
    describe('routes', function () {
        ...
        it('should have GET route for JSON list of commands for remote', function (done) {
            assert(request(app).get('/remotes/dummy_Xbox360.json').expect(200, done));
        });
        ...
        it('should have POST route for sending a command', function (done) {
            assert(request(app).post('/remotes/dummy_SonyTV/Power').expect(200, done));
        });

        it('should have POST route to start repeatedly sending a command', function (done) {
            assert(request(app).post('/remotes/dummy_SonyTV/VolumeUp/send_start').expect(200, done));
        });

        it('should have POST route to stop repeatedly sending a command', function (done) {
            assert(request(app).post('/remotes/dummy_SonyTV/VolumeUp/send_stop').expect(200, done));
        });

        it('should return 404 for sending command of an unknown remote', function (done) {
            assert(request(app).get('/remotes/DOES_NOT_EXIST/Power').expect(404, done));
        });

        it('should return 404 for sending command of an unknown key', function (done) {
            assert(request(app).get('/remotes/dummy_SonyTV/DOES_NOT_EXIST').expect(404, done));
        });

        it('should return 404 for sending start command of an unknown remote', function (done) {
            assert(request(app).get('/remotes/DOES_NOT_EXIST/Power/send_start').expect(404, done));
        });

        it('should return 404 for sending start command of an unknown key', function (done) {
            assert(request(app).get('/remotes/dummy_SonyTV/DOES_NOT_EXIST/send_start').expect(404, done));
        });

        it('should return 404 for sending stop command of an unknown remote', function (done) {
            assert(request(app).get('/remotes/DOES_NOT_EXIST/Power/send_stop').expect(404, done));
        });

        it('should return 404 for sending stop command of an unknown key', function (done) {
            assert(request(app).get('/remotes/dummy_SonyTV/DOES_NOT_EXIST/send_stop').expect(404, done));
        });


        // Sending macros
        it('should have POST route for sending a macro', function (done) {
            assert(request(app).post('/macros/Play Xbox 360').expect(200, done));
        });

        it('should return 404 for sending unknown macro', function (done) {
            assert(request(app).get('/macrosV/DOES_NOT_EXIST').expect(404, done));
        });
    ...
    describe('json api', function () {
        ...
        it('should return a list of all commands for a remote when /remotes/:remote.json is accessed', function (done) {
            request(app)
                .get('/remotes/dummy_Xbox360.json')
                .end(function (err, res) {
                    assert.equal(res.status, 200);
                    done();
                });
